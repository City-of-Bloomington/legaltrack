...
@WebServlet(urlPatterns = {"/LegalFileServ"})
public class LegalFileServ extends TopServlet{

    static String attachment_path = server_path2;
    static final long serialVersionUID = 24L;
    static Logger logger = LogManager.getLogger(LegalFileServ.class);
	...
	//
	String ext = "", old_name="";
	LegalFile legalFile = new LegalFile(debug);
	...
	try{
		...
		while (iter.hasNext()) {
			...
			if (name.equals("id")){  
			    id = value;
			    legalFile.setId(value);
			}
			else if (name.equals("notes")) {
			    notes=value;
			    legalFile.setNotes(value);
			}
			else if (name.equals("cid")){ 
			    cid = value;
			    legalFile.setCid(value);
			}
			...
			if (oldName != null && !oldName.equals("")) {
				...
				// get a new name
				//
				legalFile.setOldName(old_name);
				legalFile.composeName(ext);
				newFile = legalFile.getName();
				if(!newFile.equals("")){
				    saveDirectory = legalFile.getFullPath(attachment_path, ext);
	...
	if(action.equals("Update")){
	    String back = legalFile.doUpdate();
	    if(!back.equals("")){
		message += back;
		success = false;
	    }
	    ...
	    String back = legalFile.doDelete();
	    if(!back.equals("")){
		message += back;
		success = false;
	    }
	    ...
	    String back = legalFile.doSelect();
	    String filename = legalFile.getName();
	    String filePath = legalFile.getPath(attachment_path);
	    filePath += filename;
	    doDownload(req, res, filePath, legalFile);
	    return;
	}
	else if(action.equals("") && !id.equals("")){
	    String back = legalFile.doSelect();
	    cid = legalFile.getCid();
	}
	List<LegalFile> files = null;
	if(!cid.equals("")){
	    LegalFileList rfl = new LegalFileList(debug, cid);
	    String back = rfl.find();
	    if(!back.equals("")){
		message += back;
	    }
	    else{
		List<LegalFile> ones = rfl.getFiles();
	...
	out.println("<tr><td><label>Related Case :</label><a href=\""+url+"CaseServ?id="+cid+"\">"+cid+"</a></td></tr>");						
	if(id.equals("")){
	    out.println("<tr><td>"+
	    ...
	    out.println("<tr><td><label>Date: </label>"+legalFile.getDate()+"</td></tr>");
	
	    out.println("<tr><td><label>File: </label> <a href=\""+url+"LegalFileServ?id="+id+"&action=download\"> "+legalFile.getOldName()+"</a> </td></tr>");
	    if(legalFile.hasNotes()){
		out.println("<tr><td><label>Notes: </label>"+legalFile.getNotes()+"</td></tr>");
	    }
	    out.println("<tr>");
	...
	try{
	    ...
	    response.setHeader("Content-Disposition", "attachment; filename=\"" + (legalFile.getOldName().equals("")?legalFile.getName():legalFile.getOldName()) + "\"");
	    //
	    // Prepare streams.
	    //
            // Open streams.
